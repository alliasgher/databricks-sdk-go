...
func buildCliCommand(ctx context.Context, cliPath string, cfg *Config, ver cliVersion) []string {
	...
	if ver.AtLeast(cliVersionForForceRefresh) {
		...
		logger.Warnf(ctx, "Databricks CLI %s does not support --force-refresh (requires >= %s). The CLI's token cache may provide stale tokens.", ver, cliVersionForForceRefresh)
	}
